...
internal fun parsePartTypeFromJson(obj: JsonObject, index: Int): PartType {
    val code = obj["code"]?.jsonPrimitive?.content
        ?: throw RemoteParsingException("partTypes[$index]: campo 'code' mancante")
    val label = obj["label"]?.jsonPrimitive?.content
        ?: throw RemoteParsingException("partTypes[$index]: campo 'label' mancante")
    val peopleCount = obj["peopleCount"]?.jsonPrimitive?.int
        ?: throw RemoteParsingException("partTypes[$index]: campo 'peopleCount' mancante")
    val sexRuleStr = obj["sexRule"]?.jsonPrimitive?.content
        ?: throw RemoteParsingException("partTypes[$index]: campo 'sexRule' mancante")
    return PartType(
